...
use monoio_http::{h1::payload::{fixed_payload_pair, Payload, FramedPayloadRecvr}, common::{body::{Body, HttpBody, FixedBody}, error::HttpError}, h2::RecvStream};
...
where
    B: Body<Data = Bytes, Error = HttpError> + From<RecvStream> + From<FramedPayloadRecvr> + 'static + FixedBody<BodyType = B>,
    HttpBody: From<B> 
{
    ...
    pub async fn send_json<T: serde::Serialize>(self, data: &T) -> crate::Result<ClientResponse<B>> {
